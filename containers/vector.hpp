...
/*   Updated: 2022/04/26 11:31:41 by mdaillet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	public:
		...
		void assign(typename ft::enable_if<!ft::is_integral<InputIt>::value, InputIt>::type first, InputIt last)
		{
			...
			if (n > _capacity)
				new_cap = n;
			pointer n_array = _allocator.allocate(new_cap);
			for (size_type i = 0; first != last; i++)
			{
				_allocator.construct(n_array + i, *first);
				first++;
			}
			_allocator.deallocate(_array,  _capacity);
			_capacity = new_cap;
		...
		void assign(size_type n, const T &val) // (2)Fill a vector with a value val
		{
			...
			pointer n_array = _allocator.allocate(new_cap);
			for (size_type i = 0; i != n; i++)
				_allocator.construct(n_array + i, val);
			_allocator.deallocate(_array,  _capacity);
			_capacity = new_cap;
			_size = n;
			_array = n_array;
		...
		iterator erase(iterator position) // (1) Erase a precise position
		{
			...
			for (iterator tmp = begin(); tmp != position; tmp++)
				distance++; // Calculating offset of the position
		...
		iterator erase(iterator first, iterator last) // Erase a range in the vector
		{
			...
			for (iterator tmp = begin(); tmp != first; tmp++)
				start++; // Calculating offset of the position
			difference_type len = 0;
			for (iterator tmp = first; tmp != last; tmp++)
				len++; // Calculating offset of the position
			size_type i;
		...
		iterator insert(iterator pos, const T &val) // (1) Insert one val at a precise position
		{
			...
			difference_type start = 0;
			for (iterator tmp = begin(); tmp != pos; tmp++)
				start++; // Calculating offset of the position
		...
		{// had a scry segfault so decided not to modify old _array, and to create a new one with the right size and fulfill it ze
			if (n == 0)
				return ;
			...
			if (_size + n > _capacity)
				new_cap = _capacity * 2 >= _size + n ? _capacity * 2 : _size + n; // See capacity extension behavior
			else
				new_cap = _capacity;
			pointer n_array = _allocator.allocate(new_cap);
			size_type i = 0;
			for (iterator tmp = begin(); tmp != pos; tmp++)
			{
				_allocator.construct(n_array + i, *(_array + i));
				i++;
			}
			size_type n2 = 0;
			for (; n2 != n; n2++)
				_allocator.construct(n_array + i + n2, val);
			for (; i != _size; i++)
				_allocator.construct(n_array + i + n2, *(_array + i));
